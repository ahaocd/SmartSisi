...
void WakeWordDetect::Initialize(int channels, bool reference) {
    ...
    const auto ns_mode = reference_ ? NS_MODE_NET : NS_MODE_SSP;
    ...
    ESP_LOGI(TAG, "Wake AFE config: channels=%d ref=%d aec=%d vad=%d vad_mode=%d ns_mode=%d",
             channels_, ref_num, reference_ ? 1 : 0, 1, (int)VAD_MODE_0, (int)ns_mode);
