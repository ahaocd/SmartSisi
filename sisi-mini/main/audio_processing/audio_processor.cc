...
void AudioProcessor::Initialize(int channels, bool reference) {
    ...
    const bool enable_aec = reference_;
    const bool enable_vad = !enable_aec;
    const auto ns_mode = enable_aec ? NS_MODE_NET : NS_MODE_SSP;
    ...
    ESP_LOGI(TAG, "AFE config: channels=%d ref=%d aec=%d vad=%d ns_mode=%d",
             channels_, ref_num, enable_aec ? 1 : 0, enable_vad ? 1 : 0, (int)ns_mode);
...
void AudioProcessor::AudioProcessorTask() {
    ...
    while (true) {
        ...
        if (res == nullptr || res->ret_value == ESP_FAIL) {
            if (res != nullptr) {
                ESP_LOGD(TAG, "Audio processor fetch returned: %d (normal during idle)", res->ret_value);
            }
            continue;
        }
