...
NoAudioCodecDuplex::NoAudioCodecDuplex(int input_sample_rate, int output_sample_rate, gpio_num_t bclk, gpio_num_t ws, gpio_num_t dout, gpio_num_t din, bool input_reference) {
    duplex_ = true;
    ...
    ESP_LOGI(TAG, "Duplex channels created (input_reference=%s, input_channels=%d)",
             input_reference_ ? "true" : "false", input_channels_);
}

ATK_NoAudioCodecDuplex::ATK_NoAudioCodecDuplex(int input_sample_rate, int output_sample_rate, gpio_num_t bclk, gpio_num_t ws, gpio_num_t dout, gpio_num_t din, bool input_reference) {
    duplex_ = true;
    input_reference_ = input_reference;
    input_channels_ = input_reference_ ? 2 : 1;
    input_sample_rate_ = input_sample_rate;
    ...
    ESP_ERROR_CHECK(i2s_channel_init_std_mode(tx_handle_, &tx_cfg));
    ESP_ERROR_CHECK(i2s_channel_init_std_mode(rx_handle_, &rx_cfg));
    ESP_LOGI(TAG, "ATK duplex channels created (input_reference=%s, input_channels=%d)",
             input_reference_ ? "true" : "false", input_channels_);
}
