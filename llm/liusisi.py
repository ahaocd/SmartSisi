...
_llm_override_local = threading.local()


def set_llm_override(override):
    """Set per-thread LLM override for current interaction only."""
    ...
    provider_id = str(override.get("provider_id") or "override").strip()
    api_style = str(override.get("api_style") or "").strip().lower()
    if api_style not in ("openai", "anthropic"):
        api_style = "anthropic" if model.lower().startswith("claude-") else "openai"
    if not base_url or not api_key or not model:
        clear_llm_override()
        return
    _llm_override_local.value = {
        "base_url": base_url,
        "api_key": api_key,
        "model": model,
...
def clear_llm_override():
    if hasattr(_llm_override_local, "value"):
        try:
            del _llm_override_local.value
        except Exception:
            _llm_override_local.value = None


def _get_llm_override():
    raw = getattr(_llm_override_local, "value", None)
    ...
    return {
        ...
        "api_key": api_key,
        "model": model,
        "provider_id": str(raw.get("provider_id") or "override").strip(),
        "api_style": api_style,
    }
...
## 输出格式（最高优先级）##
1. 复杂任务：回复文本中插入在结尾 {执行复杂任务}
2. 唱歌请求：开始或者结尾文本中插入 {曲名}
3. 切换系统：文本回复的结尾中插入 {柳叶} 或 {妹妹}
4. 其他：仅输出纯文本，不包含任何{},不能同时存在两个{}

## 复杂任务判定（必须触发）##
- 用户明确说“执行复杂任务”时，必须回复结尾插入{执行复杂任务}。
...
def _resolve_llm_api_style(llm_cfg):
    if not isinstance(llm_cfg, dict):
        return "openai"
    style = str(llm_cfg.get("api_style") or "").strip().lower()
...
def _to_text_for_prompt(value):
    ...
    if isinstance(value, dict):
        return _to_text_for_prompt([value])
    return str(value or "").strip()


def _normalize_anthropic_content(value):
    if isinstance(value, str):
        text = value.strip()
        return text if text else None

    if isinstance(value, dict):
        return _normalize_anthropic_content([value])
...
def get_llm_cfg(persona=None):
    override = _get_llm_override()
    if override:
        return {
            "api_key": override["api_key"],
            "base_url": override["base_url"],
            "model": override["model"],
            "api_style": _resolve_llm_api_style(override),
        }
    cfg_data = cfg.get_persona_llm_config(persona or get_current_system_mode()) or {}
    if not isinstance(cfg_data, dict):
        return {"api_key": "", "base_url": "", "model": "", "api_style": "openai"}
    normalized = dict(cfg_data)
    normalized["api_style"] = _resolve_llm_api_style(normalized)
    return normalized
...
def send_llm_request(session, data, llm_cfg):
    ...
    Returns:
        dict: 包含回复文本和语气的字典
    """
    api_style = _resolve_llm_api_style(llm_cfg)
    ...
    try:
        if api_style == "anthropic":
            ...
            headers = {
                ...
                "x-api-key": str(llm_cfg.get("api_key") or ""),
                "anthropic-version": "2023-06-01",
            }

            raw_messages = request_data.get("messages") if isinstance(request_data.get("messages"), list) else []
            system_parts = []
            anthropic_messages = []
            for item in raw_messages:
                ...
                if role == "system":
                    text = _to_text_for_prompt(content)
                    if text:
                ...
                if role not in ("user", "assistant"):
                    role = "user"
                normalized_content = _normalize_anthropic_content(content)
                if not normalized_content:
                    continue
                anthropic_messages.append({"role": role, "content": normalized_content})

            if not anthropic_messages:
                anthropic_messages = [{"role": "user", "content": "你好"}]

            payload = {
                "model": llm_cfg["model"],
                "messages": anthropic_messages,
                "temperature": float(request_data.get("temperature", 1.0) or 1.0),
                "max_tokens": int(request_data.get("max_tokens", 200) or 200),
                "top_p": float(request_data.get("top_p", 0.9) or 0.9),
            }
            ...
            for attempt in range(2):
                ...
                if response.status_code != 520:
                    break
                util.log(2, f"[LLM] Anthropic上游520，第{attempt + 1}次重试")
            response.raise_for_status()
            result = response.json()
            blocks = result.get("content")
            if not isinstance(blocks, list):
                util.log(2, "[LLM] Anthropic响应数据不完整")
                return {"text": "让我想想该怎么回答...", "tone": "gentle"}
            text_parts = []
            for block in blocks:
            ...
            if not content:
                util.log(2, "[LLM] Anthropic文本内容为空")
                return {"text": "让我想想该怎么回答...", "tone": "gentle"}
        else:
        ...
        try:
            ...
            parsed = urlparse(str(llm_cfg.get("base_url") or ""))
            if parsed.hostname in ("127.0.0.1", "localhost", "0.0.0.0") and parsed.port == 8000:
                util.log(2, "[LLM] 检测到本地8000端口调用，可能与ESP32 WebSocket服务冲突（POST会被当成无效握手）")
        except Exception:
            pass
...
async def request_openai_api_async(text: str, uid=0, observation: str = ''):
    ...
    try:
        ...
        llm_cfg = get_llm_cfg()
        stop_tokens = ["ASSISTANT:", "USER:", "助手：", "用户：", "系统："]
        data = {
            "model": llm_cfg["model"],
            "messages": [
...
def question(content, uid=0, observation="", audio_context=None, brain_prompts=None, speaker_info=None, mode_switched: bool = False):
    ...
    if isinstance(content, dict) and isinstance(content.get("llm_user_content_parts"), list):
        llm_user_content_parts = content.get("llm_user_content_parts")
...
"""
        try:
            ...
            # 分段规则：去掉 "~"，加入中英文逗号/分号/冒号
            strong_punct_pattern = re.compile(r'[。！？!?]$')
            weak_punct_pattern = re.compile(r'[，,；;：:]$')
            pause_base_ms = {
                "，": 170, ",": 160,
                "；": 220, ";": 220,
                "：": 200, ":": 190,
            ...
            def _enqueue_pc_silence(ms: int, label: str = "punct_pause") -> bool:
                """向PC播放队列插入一段静音，用于控制段间停顿。"""
                try:
                    from utils.pc_stream_queue import get_pc_stream_queue
                    pc_queue = get_pc_stream_queue()
                    sink = pc_queue.enqueue_stream(label=label)
                    total_bytes = int(max(0, ms) * 16 * 2)  # 16kHz * 16bit * mono
                    if total_bytes <= 0:
                        sink.finish()
                        return True
                    chunk_size = 4096
                    zero_chunk = b"\x00" * min(chunk_size, total_bytes)
                    left = total_bytes
                    while left > 0:
                        n = min(left, len(zero_chunk))
                        sink.push(zero_chunk[:n])
                        left -= n
                    sink.finish()
                    return True
                except Exception as _qe:
                    util.log(2, f"[NLP-Stream] 插入静音失败: {_qe}")
                    return False

            def _compute_dynamic_pause_ms(text: str) -> int:
                """基于标点和句长计算动态停顿时长。"""
                t = (text or "").strip()
                if not t:
                    return 0
                base = pause_base_ms.get(t[-1], 0)
                if base <= 0:
                    return 0
                ...
                if style_lower in ("fast", "energetic"):
                    base = int(base * 0.80)
                elif style_lower in ("whisper", "calm", "gentle"):
                    base = int(base * 1.05)

                return max(80, min(460, int(base)))

            def _apply_segment_pause(text: str):
                pause_ms = _compute_dynamic_pause_ms(text)
                if pause_ms <= 0:
                    return
                if not _esp32_connected():
                    if _enqueue_pc_silence(pause_ms, label=f"punct_pause:{pause_ms}ms"):
                        return
                time.sleep(pause_ms / 1000.0)

            def try_emit(force=False):
                nonlocal seg_buf, last_emit, brace_depth, emitted_any, emitted_segment_count
                now = time.time()
                stripped_buf = (seg_buf or "").strip()
                ready_by_strong = bool(stripped_buf and strong_punct_pattern.search(stripped_buf))
                ready_by_weak = bool(stripped_buf and weak_punct_pattern.search(stripped_buf))
                ready_by_punct = ready_by_strong or ready_by_weak

                # 逗号类分段使用动态阈值，避免切得太碎
                if not force and ready_by_weak:
                    plain_for_len = re.sub(r'\{([A-Za-z0-9_\u4e00-\u9fff]+)\}', '', stripped_buf)
                    plain_len = len(re.sub(r"\s+", "", plain_for_len))
                    dynamic_min_len = 8 if emitted_segment_count == 0 else 12
                    dynamic_min_interval = 0.18 + min(0.20, plain_len / 100.0)
                    if plain_len < dynamic_min_len or (now - last_emit) < dynamic_min_interval:
                        return
                        ...
                        if item_type == "text":
                            ...
                            if sisi_core:
                                try:
                                    ...
                                    if not force:
                                        _apply_segment_pause(cleaned_text)
            ...
            # 强制按UTF-8解析SSE
            chunk_count = 0  # 文本token chunk计数
            empty_chunk_count = 0  # 非文本/空chunk计数
            music_status_sent = set()  # 🎵 记录已发送的音乐状态，避免重复
            
            def _extract_text_token(delta: dict) -> str:
                """从不同供应商的流式delta结构中提取可播报文本。"""
                ...
                if isinstance(content, list):
                    ...
                    for item in content:
                        ...
                        if isinstance(item, dict):
                            # 兼容分段内容结构
                            text_piece = item.get('text') or item.get('content') or item.get('value')
                            if isinstance(text_piece, str):
                                parts.append(text_piece)
                    return "".join(parts)
                for alt_key in ('text', 'output_text'):
            ...
            for raw_line in resp.iter_lines(decode_unicode=False):
                ...
                if line.startswith('data: '):
                    ...
                    if payload == "[DONE]":
                        util.log(
                            1,
                            f"[NLP-Stream调试] 🏁 收到[DONE]，流式结束，已收到{chunk_count}个文本chunk，空chunk={empty_chunk_count}，全文: {full_text}",
                        )
                        break
                    delta = {}
                    try:
                        ...
                        token = _extract_text_token(delta)
                    ...
                    if not token:
                        empty_chunk_count += 1
                        if empty_chunk_count <= 3 or empty_chunk_count % 100 == 0:
                            non_text_keys = []
                            if isinstance(delta, dict):
                                non_text_keys = [k for k, v in delta.items() if k != "content" and v not in (None, "", [], {})]
                            util.log(
                                1,
                                f"[NLP-Stream调试] ⏭️ 空chunk#{empty_chunk_count}（非文本字段={non_text_keys if non_text_keys else '无'}）",
                            )
                        continue
                    chunk_count += 1
                    if chunk_count <= 3 or chunk_count % 20 == 0:
                        util.log(1, f"[NLP-Stream调试] 📦 文本chunk#{chunk_count}: token长度={len(token)}")
                    full_text += token
            ...
            util.log(
                1,
                f"[NLP-Stream调试] 🎯 流式播放结束，emitted_any={emitted_any}, 全文长度={len(full_text)}, 文本chunk数={chunk_count}, 空chunk数={empty_chunk_count}",
            )
            try:
                from core import sisi_booter
    ...
    try:
        ...
        # Build dynamic brain prompt content from pre-brain output.
        brain_context = ""
        if brain_prompts:
        ...
        # 构建用户消息：文本/多模态共用同一主链（系统提示词、历史、记忆保持不变）
        if speaker_info and speaker_info.get('real_name'):
            speaker_name = speaker_info['real_name']

        llm_cfg = get_llm_cfg()
        llm_api_style = _resolve_llm_api_style(llm_cfg)
        if (
            llm_api_style in ("openai", "anthropic")
            and isinstance(llm_user_content_parts, list)
            and llm_user_content_parts
        ):
            user_message = llm_user_content_parts
            util.log(1, f"[NLP-Multimodal] 直传多模态内容块: {len(llm_user_content_parts)}")
        else:
        ...
        if isinstance(user_message, list):
            block_types = []
            for _item in user_message:
                if isinstance(_item, dict):
                    block_types.append(str(_item.get("type") or ""))
            user_message_debug = f"[multimodal blocks={len(user_message)} types={block_types}]"
        else:
            user_message_debug = user_message
        util.log(1, f"[NLP-完整调试] 💬 User Message: {user_message_debug}")
        ...
        if use_stream and llm_api_style != "openai":
            util.log(1, f"[NLP-Stream] 当前LLM接口风格={llm_api_style}，切换为非流式主链请求")
            use_stream = False
        ...
        if use_stream:
            ...
            if streamed_text:
                ...
                # 流式失败：自动回退到非流式，避免空回复
                util.log(2, "[NLP-Stream] 流式失败，回退到非流式补偿")
                response = send_llm_request(session, {"messages": messages, "stop": stop_tokens}, llm_cfg)
                if response and isinstance(response, dict):
                    answer = response["text"].strip() or ""
                    style = response.get("tone", style_stream or "gentle")
