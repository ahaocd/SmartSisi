...
def get_whisper_model():
    ...
    try:
        ...
        # 检查是否已下载（HuggingFace 缓存格式）
        # 可能的路径：faster-whisper-{size} 或 models--Systran--faster-whisper-{size}
        local_paths = [
            model_dir / f"faster-whisper-{model_size}",
        ...
        for path in local_paths:
            if path.exists():
                ...
                if (path / "snapshots").exists():
                    model_exists = True
                    print(f"[INFO] 找到本地模型: {path.name}")
                    break
                # 或者是直接的模型文件格式（有 model.bin）
                elif (path / "model.bin").exists():
                    model_exists = True
                    print(f"[INFO] 找到本地模型: {path.name}")
                    break
        
        if model_exists:
            print(f"[INFO] 加载 Whisper 模型(faster-whisper-{model_size})...")
            # 直接用模型名称加载，faster-whisper 会自动找到缓存
...
直接返回优化后的文本，不要任何解释。`
        ...
        async function recognizeAudio() {
            ...
            if (file.size > 1 * 1024 * 1024) {
                showMsg(msgDiv, `文件 ${fileSizeMB} MB，识别中，请耐心等待（首次加载模型需要3-5分钟，后续会更快）...`, true);
            } else {
                hideMsg(msgDiv);
            }
            ...
            try {
                ...
                const timeoutId = setTimeout(() => controller.abort(), 600000); // 600秒(10分钟)超时，首次加载模型需要更长时间
                ...
                if (e.name === 'AbortError') {
                    showMsg(msgDiv, '识别超时（超过10分钟），请检查音频文件或联系管理员', false);
                } else {
...
def api_voices():
    ...
    try:
        ...
        for voice in all_clones:
            ...
            # 优先从本地记录判断模型类型
            model_type = None
            for name, info in local_voices.items():
                if isinstance(info, dict) and info.get('uri') == uri:
            ...
            if not model_type:
                ...
                if 'IndexTTS' in model or 'IndexTeam' in model:
            ...
            if model_type == 'indextts2':
                ...
                # cosyvoice 和 moss 都归到 cosyvoice 列表
...
def api_upload():
    ...
    try:
        ...
        if success and uri:
            # 保存到本地voices.json，记录模型类型
            voices = load_voices()
            voices[name] = {
                "uri": uri,
                "model": tts_model,
                "model_type": model_type,  # 记录原始模型类型（cosyvoice/indextts2/moss）
                "ref_text": ref_text,
                "time": time.strftime("%Y-%m-%d %H:%M")
            }
            save_voices_db(voices)
            print(f"[INFO] 上传成功并保存到本地: {uri}")
