...
class AutoCookieManager:
    ...
    def __init__(
        ...
        refresh_interval: int = 172800,  # 默认48小时（2天）
        headless: bool = False
    ):
        """
        初始化Cookie管理器
    ...
    def _need_refresh(self) -> bool:
        ...
        # 检查是否过期（改为7天）
        if self.current_cookies.is_expired(max_age_hours=168):  # 7天 = 168小时
            logger.info("Cookies已过期（超过7天），需要刷新")
            return True
    ...
    async def _refresh_cookies(self):
        ...
        try:
            ...
            if not is_logged_in:
                ...
                logger.info("✅ 检测到已登录，等待Cookie完全加载...")
                await asyncio.sleep(8)
                try:
                    logger.info("🔄 刷新页面获取完整Cookie...")
                    await page.reload(wait_until='domcontentloaded', timeout=30000)
                    await asyncio.sleep(5)
                except Exception as e:
                    logger.warning(f"⚠️ 页面刷新失败: {e}")
