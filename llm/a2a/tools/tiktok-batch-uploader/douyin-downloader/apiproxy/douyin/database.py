...
class DataBase(object):
    def __init__(self, db_path: str = None):
        # 修复：使用绝对路径，默认在douyin-downloader目录下
        if db_path is None:
            # 获取当前文件所在目录的父父目录（douyin-downloader/）
            current_file = Path(__file__).resolve()
            douyin_downloader_dir = current_file.parent.parent.parent
            db_path = str(douyin_downloader_dir / 'data.db')
